function [x,final_velocity,final_exact]=semi_lagr_function(lambda)
...
a=0.5;              % Propagation velocity given by the linear advection equation
N_grid=101;         % Number of grid points as specified by the question
...
Deltat=(lambda*Deltax)/a;       % Time step: derived by definition of Courant number
...
for n=1:length(t)-1                 % Keep the convention of subscripts with handout
    for i=1:(N_grid-1)              % Keep the convention of subscripts with handout
        k(1)=i-ceil(lambda);        % Capture the left side of the departure point
        x_departure=x(i)-a*Deltat;  % Evaluate the position of the departure point
...
exact(end,:)=exact(1,:);             %Periodic condition
final_exact=exact(:,end);            %Export the final slice of the exact solution
    end
